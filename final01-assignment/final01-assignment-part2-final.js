...
request('https://raw.githubusercontent.com/julianhlange/data-structures/master/final01-assignment/addressarraywithoutlatLong.json', function(error, response, body) {
  ...
  async.eachSeries(meetingswithoutlatLong, function(value, callback) {
    ...
    setTimeout(callback, 250);
      ...
      require('fs').writeFile('addressarraywithlatLong.json', JSON.stringify(meetingsData), function (err) {
        if (err) {
          console.error('error');
        }
      }
...
// then push folder to github to make addressarraywithlatLong.json available for next part