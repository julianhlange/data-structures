// week04 assignment in two parts: this is part 1
// code below is copy-paste from week03-assignment-final.js, except modify line 73 to save file type as json instead of txt
// push to github so that have json file there
// continue with part 2 at week04-assignment-part2-final.js
...
async.eachSeries(fullAddressArrayFinal, function(value, callback) {
    ...
    // testing during next class, looks like I probably needed to enter lines 43 and 44  again in Terminal because Cloud9 had forgotten API key, then use starter code with '=' character
    // var apiRequest = 'https://maps.googleapis.com/maps/api/geocode/json?address=' + value.split(' ').join('+') + '&key=' + apiKey;
    ...
    require('fs').writeFile('week04-assignment/m04addressarray.json', JSON.stringify(meetingsData), function (err) {
      if (err) {
        console.error('error');
      }
    }
